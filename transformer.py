...
def split_heads(A, n_heads):
    return A.reshape(shape_list(A)[:-1]+[shape_list(A)[-1]//n_heads, n_heads])

def merge_heads(A):
    return A.reshape(shape_list(A)[:-2]+[shape_list(A)[-2]*shape_list(A)[-1]])

def layer_norm(x, L):
    # x = (batch, seq, data)
    mean = (torch.einsum('bij->b', x)/(L.reshape(-1)*x.shape[2])).reshape(-1,1,1)
    std_dev = ((torch.einsum('bij->b', (x-mean)**2)**(1/2))/(L.reshape(-1)*x.shape[2])).reshape(-1,1,1)
    x = (x-mean)/std_dev
    return x
...
class Attn(nn.Module):
    ...
    def forward(self, x):
        Q, K, V = torch.matmul(x, self.Wq)+self.Bq, torch.matmul(x, self.Wk), torch.matmul(x, self.Wv)
...
class Transformer(nn.Module):
    def __init__(self, d_model=30, n_output=4, n_heads=3, p=.1, device=device):
        ...
        self.seq = nn.ModuleList([AttnBlock(d_model, n_heads, device) for i in range(10)])
